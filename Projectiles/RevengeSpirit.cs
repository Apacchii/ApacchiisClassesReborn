...
{
    public class RevengeSpirit : ModProjectile
    {
        bool canHome = false;
        int timeToHit = 3;
        ...
        public override void AI()
        {
            ...
            var dust = Dust.NewDustPerfect(Projectile.Center, DustID.GemRuby, new Vector2(Projectile.velocity.X * .8f, Projectile.velocity.Y * .8f), 110, Color.Orange, 1.75f);
            dust.noGravity = true;
            ...
            if (canHome)
            {
                ...
                if (Projectile.ai[1] == 0f)
                {
                    ...
                    {
                        if (Main.npc[target].CanBeChasedBy(this, false) && (Projectile.ai[1] == 0f || Projectile.ai[1] == (float)(target + 1)) && !Main.npc[target].boss)
                        {
        ...
        {
            if (Projectile.timeLeft <= 60 * 4 - timeToHit && !target.townNPC && !target.friendly && target.type != NPCID.TargetDummy && !target.boss)
                return true;
            else
                return false;
        }
