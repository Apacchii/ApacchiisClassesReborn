...
namespace ApacchiisClassesMod2.Items.Classes
{
	public class Plague : ModItem
	{
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            ...
            if (Player.controlUp)
            {
                classStats = $"+{level * stat1 * 100:F2}% {Language.GetTextValue("Mods.ApacchiisClassesMod2." + damageType)}\n" +
                             $"+{level * stat2 * 100:F2}% {Language.GetTextValue("Mods.ApacchiisClassesMod2.AbilityPower")}\n" +
                             $"+{level * stat3 * 100:F2}% Minion Crit Chance";
            }
            else
            {
                classStats = $"+{level * stat1 * 100 * modPlayer.classStatMultiplier:F2}% {Language.GetTextValue("Mods.ApacchiisClassesMod2." + damageType)}\n" +
                             $"+{level * stat2 * 100 * modPlayer.classStatMultiplier:F2}% {Language.GetTextValue("Mods.ApacchiisClassesMod2.AbilityPower")}\n" +
                             $"+{level * stat3 * 100 * modPlayer.classStatMultiplier:F2}% Minion Crit Chance";
            }
        ...
        public override void UpdateAccessory (Player Player, bool hideVisual)
		{
            ...
            //A1
            int plaguedDuration = acmPlayer.plagueDebuffDuration;
            if (acmPlayer.plagueTalent_9 == "R") plaguedDuration += 2;
            int infectContactDamage = acmPlayer.plagueInsectDamageBase;
            ...
            //A2
            int burstRange = acmPlayer.plagueInfectionRange;
            if (acmPlayer.plagueTalent_2 == "R") burstRange += 50;
            if (acmPlayer.plagueTalent_6 == "R") burstRange += 50;
            int burstDamage = acmPlayer.plagueInfectionBurstBase;
