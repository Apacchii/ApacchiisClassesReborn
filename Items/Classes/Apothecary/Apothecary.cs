...
{
    public class Apothecary : ModItem
    {
        int classLevel;
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            ...
            TooltipLine AbilityPreview = new TooltipLine(Mod, "AbilityPreview",
                $"-(P: {Language.GetTextValue("Mods.ApacchiisClassesMod2.Apothecary.P_Name")})-\n" +
                $"{Language.GetTextValue("Mods.ApacchiisClassesMod2.Plague.P_Prev")}\n" +
                $"-(A1: {Language.GetTextValue("Mods.ApacchiisClassesMod2.Apothecary.A1_Name")})-\n" +
                $"{Language.GetTextValue("Mods.ApacchiisClassesMod2.Plague.A1_Prev")}\n" +
                $"-(A2: {Language.GetTextValue("Mods.ApacchiisClassesMod2.Apothecary.A2_Name")})-\n" +
                $"{Language.GetTextValue("Mods.ApacchiisClassesMod2.Plague.A2_Prev")}\n" +
                $"-(Ult: {Language.GetTextValue("Mods.ApacchiisClassesMod2.Apothecary.Ult_Name")})-\n" +
            ...
            if (Player.controlUp)
            {
                classStats = $"+{level * stat1 * 100:F2}% Generic Damage\n" +
                             $"+{level * stat2 * 100:F2}% {Language.GetTextValue("Mods.ApacchiisClassesMod2.MaxHealth")}\n" +
                             $"+{level * stat3 * 100:F2}% {Language.GetTextValue("Mods.ApacchiisClassesMod2.Defense")}";
            }
            else
            {
                classStats = $"+{level * stat1 * 100 * modPlayer.classStatMultiplier:F2}% Generic Damage\n" +
                             $"+{level * stat2 * 100 * modPlayer.classStatMultiplier:F2}% {Language.GetTextValue("Mods.ApacchiisClassesMod2.MaxHealth")}\n" +
            ...
            foreach (TooltipLine line in tooltips)
                if (line.Mod == "Terraria" && line.Name == "Equipable")
                    line.Text = $"{Language.GetTextValue("Mods.ApacchiisClassesMod2.EquipableGeneralist")}";
        ...
        public override void UpdateAccessory(Player Player, bool hideVisual)
        {
            ...
            acmPlayer.equippedClass = "Apothecary";
            acmPlayer.ultChargeMax = 2400;
            acmPlayer.ability1MaxCooldown = 0;
            ...
            acmPlayer.P_Name = Language.GetTextValue("Mods.ApacchiisClassesMod2.Apothecary.P_Name");
            acmPlayer.P_Desc = $"";
            acmPlayer.P_Effect_1 = $"";
            acmPlayer.P_Effect_2 = $"";

            acmPlayer.A1_Name = Language.GetTextValue("Mods.ApacchiisClassesMod2.Apothecary.A1_Name");
            acmPlayer.A1_Desc = $"";
            acmPlayer.A1_Effect_1 = $"";
            acmPlayer.A1_Effect_2 = $"";

            acmPlayer.A2_Name = Language.GetTextValue("Mods.ApacchiisClassesMod2.Apothecary.A2_Name");
            acmPlayer.A2_Desc = $"";
            acmPlayer.A2_Effect_1 = $"";
            acmPlayer.A2_Effect_2 = $"";

            acmPlayer.Ult_Name = Language.GetTextValue("Mods.ApacchiisClassesMod2.Apothecary.Ult_Name");
            acmPlayer.Ult_Desc = $"";
            acmPlayer.Ult_Effect_1 = $"";
            acmPlayer.Ult_Effect_2 = $"";

            acmPlayer.aghanimsText = "- Increases all mixes max charges to 15\n" +
                                     "- Grants a 8% chance when crafting a mix to craft another random mix";
        }
